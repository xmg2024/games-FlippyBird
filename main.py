...
import pygame
# Import color module separately to avoid circular imports
try:
    from pygame.color import THECOLORS as COLORS
except ImportError:
    # Fallback colors if THECOLORS is not available
    COLORS = {
...
def init_fonts():
    ...
    try:
        ...
        try:
            ...
            # If all else fails, create dummy font objects
            class DummyFont:
                def __init__(self, size=30):
                    self.size = size
                    
                def render(self, text, antialias, color):
                    # Create a simple surface as fallback
                    surf = pygame.Surface((len(text) * max(10, self.size//2), self.size))
                    surf.fill(COLORS['white'])
                    return surf
            
            return DummyFont(50), DummyFont(120)
...
if __name__ == "__main__":
    ...
    try:
        ...
        birdImg = birdImg.convert_alpha()
    except pygame.error as e:
        print(f"Could not load image {birdPath}: {e}")
        print("Creating a simple rectangle as fallback...")
        # Create a simple colored rectangle as fallback
        birdImg = pygame.Surface((70, 70), pygame.SRCALPHA)
        birdImg.fill((255, 255, 0, 200))  # Yellow bird with some transparency
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:  # Use pygame constant instead of chr()
                    jump = True
